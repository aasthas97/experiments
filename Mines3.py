"""Round 3 of Minesweeper for Decision-Making"""
...
outputFile = open(name + "_mine3" + "_" + trial_type + trial_num + '.txt', 'w+')

def reset(): # reset game
    print(open('instructions3.txt', 'r').read())
...
def play (b, k, startTime):
    ...
    if v != 0 and v != "*":
        ...
        NonDiagonalMines = CheckNonDiagonal(r, c, b)
        if DiagonalMines + NonDiagonalMines == 2: #mines in both
            print ('Mine in diagonal position?: Yes\nMine in non-diagonal position?: Yes\n')
        elif DiagonalMines + NonDiagonalMines == 1: # mine in one
            if DiagonalMines == 1:
                print('Mine in diagonal position?: Yes\nMine in non-diagonal position?: No\n')
            elif NonDiagonalMines == 1:
...
def CheckNonDiagonal(r, c, b):
    NonDiagonalMines = 0
    if r == 0: # first row, don't check r - 1
        if c == 0 and (b[r][c+1] == "*" or b[r+1][c] == "*"): # corner column, don't check adjacent left
            NonDiagonalMines = 1
        elif c == 5 and (b[r][c-1] == "*" or b[r+1][c] == "*"):
            NonDiagonalMines = 1
        elif c != 0 and c != 5:
            if b[r][c-1] == "*" or b[r][c+1] == "*" or b[r+1][c] == "*":
                NonDiagonalMines = 1
        else:
            pass

    elif r == 5: # last row, don't check r + 1
        if c == 0 and (b[r-1][c] == "*" or b[r][c+1] == "*"):
            NonDiagonalMines = 1
        elif c == 5 and (b[r-1][c] == "*" or b[r][c-1] == "*"):
            NonDiagonalMines = 1
        elif c != 0 and c != 5:
            if b[r-1][c] == "*" or b[r][c-1] == "*" or b[r][c+1] == "*":
                NonDiagonalMines = 1
        else:
            pass

    else: # middle rows, check above and below
        if c == 0 and (b[r-1][c] == "*" or b[r][c+1] == "*" or b[r+1][c] == "*"):
            NonDiagonalMines = 1
        elif c == 5 and (b[r-1][c] == "*" or b[r][c-1] == "*" or b[r+1][c] == "*"):
            NonDiagonalMines = 1
        elif c != 0 and c != 5:
            if b[r-1][c] == "*" or b[r][c-1] == "*" or b[r][c+1] == "*" or b[r+1][c] == "*":
                NonDiagonalMines = 1
        else:
            pass

    return NonDiagonalMines
